            ...
            if (validTarget) {
                ...
                if (childNode.type == "INSTANCE" || childNode.type == "COMPONENT" || childNode.type == "FRAME" || childNode.type == "RECTANGLE" || childNode.type == "GROUP") {
                    ...
                    switch (visualPositioning) {
                        case "Left":
                            if (!(xAlign == "Left")) {
                                const warn = new MissingConstraintsWarning(target, parent, visualPositioning, xAlign, visualPositioning)
                                lints.push(warn)
                            }
                            break;
                        case "Right":
                        ...
                        case "Center":
                            if (!(xAlign == "Center" || xAlign == "Stretch")) {
                                const warn = new MissingConstraintsWarning(target, parent, visualPositioning, xAlign, `Stretch or Center`)
                                lints.push(warn)
                            }
                            break;
                        case "Stretch":
                            if (!(xAlign == "Center" || xAlign == "Stretch" || xAlign == "Scale")) {
                                const warn = new MissingConstraintsWarning(target, parent, visualPositioning, xAlign, `Stretch, Center, or Scale`)
                                lints.push(warn)
                            }
                    }
                }
